...
export default function Footer() {
  ...
  return (
    ...
    >
      <div className="px-8 md:px-12 lg:px-20 xl:px-32 2xl:px-72 py-16 lg:py-20 xl:py-24 grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr] gap-12 lg:gap-16 xl:gap-20">
        <div className="flex flex-col gap-6 lg:gap-8">
          <img src={Logo} alt="Alcaware" className="h-8 lg:h-10 xl:h-12 w-auto object-contain object-left" />
          <div className="flex flex-col gap-1">
            <p className="font-montserrat text-sm lg:text-base xl:text-lg font-bold text-white">{t("footer.call")}</p>
            <p className="font-montserrat text-sm lg:text-base xl:text-lg text-white/60">+52 81 1635 9851</p>
          </div>
          <div className="flex flex-col gap-1">
            <p className="font-montserrat text-sm lg:text-base xl:text-lg font-bold text-white">{t("footer.contact")}</p>
            <a
              href="mailto:it@alcaware.com"
              className="font-montserrat text-sm lg:text-base xl:text-lg text-white/60 hover:text-primary transition-colors"
            >
              it@alcaware.com
            </a>
          </div>
          ...
          <div className="flex items-center gap-4 lg:gap-5 xl:gap-6">
            <a href="#" aria-label="Instagram" className="text-white/60 hover:text-primary transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 lg:w-6 lg:h-6 xl:w-7 xl:h-7" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-col gap-4 lg:gap-6">
          <p className="font-montserrat text-sm lg:text-base xl:text-lg font-bold text-white">{t("footer.info_title")}</p>
          <nav className="flex flex-col gap-3 lg:gap-4">
            {navLinks.map((item) => (
              <a
                key={item}
                href={`#${item.toLowerCase()}`}
                className="font-montserrat text-sm lg:text-base xl:text-lg text-white/60 hover:text-primary transition-colors w-fit"
              >
                {item}
              </a>
            ))}
        ...
        <div className="flex flex-col gap-4 lg:gap-6">
          <p className="font-montserrat text-sm lg:text-base xl:text-lg font-bold text-white">{t("footer.visit_title")}</p>
          <address className="font-montserrat text-sm lg:text-base xl:text-lg text-white/60 not-italic leading-relaxed">
            {t("footer.address")}
          </address>
        </div>
      </div>
      <div className="border-t border-white/10">
        <p className="text-center font-montserrat text-xs lg:text-sm xl:text-base text-white/40 py-4 lg:py-5 tracking-widest">
          {t("footer.copyright")}
        </p>
      </div>
    </footer>
